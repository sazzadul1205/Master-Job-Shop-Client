...
const EditGigModal = ({ selectedGigData, refetch }) => {
  ...
  useEffect(() => {
    if (selectedGigData?._id) {
      // Destructure relevant fields from selectedGigData
      const {
        title,
        description,
        category,
      ...
      // Format deliveryDeadline for datetime-local input
      const formattedDeliveryDeadline = deliveryDeadline
        ? new Date(deliveryDeadline).toISOString().slice(0, 16)
        : "";

      // Reset the form with destructured data & formatted fields
      reset({
        title,
        description,
        category,
        subCategory,
        tags,
        deliveryDeadline: formattedDeliveryDeadline,
        requiredSkills,
        attachments,
        isRemote,
        location,
  ...
  return (
    <div className="modal-box min-w-3xl relative bg-white rounded-lg shadow-xl w-full max-w-3xl mx-auto max-h-[90vh] p-6 text-black overflow-y-auto">
      ...
      <button
        type="button"
        onClick={() => document.getElementById("Edit_Gig_Modal").close()}
