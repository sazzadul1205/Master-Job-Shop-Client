...
const ManageJobApplications = () => {
  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        const res = await axiosPublic.get(`/JobApplications?${query}`);
        return Array.isArray(res.data) ? res.data : [];
      } catch (err) {
        if (err.response?.status === 404) return []; // no applications found
        return []; // fallback on error
      }
    },
  ...
  const JobsWithApplicants = JobsData.map((job) => {
    const applicants = JobApplicationsData.filter(
        ...
        {JobsWithApplicants?.length > 0 ? (
          ...
          <div className="w-full flex flex-col items-center justify-center py-16 px-4">
            ...
            <p className="text-sm text-gray-500 max-w-xs text-center">
              Looks like you haven&apos;t posted any jobs yet. Start posting
              jobs now to find the perfect candidates for your team!
            </p>
          </div>
        )}
      </div>
